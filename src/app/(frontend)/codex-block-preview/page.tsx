...
export default async function CapturePage({ searchParams }: {
    ...
    if (query.demo) {
        if (query.demo === "cta") block = {
            blockType: "centeredCta", heading: "Pojďme společně vyřešit vaše IT",
            link: { type: "custom", url: "/", label: "Domluvit konzultaci", appearance: "default" },
        };
    } else {
        const id = Number(query.page);
        if (![4, 5, 6, 8, 21, 22].includes(id)) notFound();
        page = await payload.findByID({ collection: "pages", id, locale: "cs", depth: 2, overrideAccess: false });
        block = page.layout[Number(query.block)];
    }
    if (!block) notFound();
    const dark = ["metricsStrip", "logoMarquee", "centeredCta", "formBlock", "hero", "homepageHero"].includes(block.blockType);
    const background = dark ? (block.blockType === "hero" ? "#111111" : "#0e0e0e") : "#fafcf8";
    return <>
        ...
        <main id="csf-capture" data-block-type={block.blockType}>
            <div><RenderBlocks blocks={[block]} page={page} locale="cs" /></div>
        </main>
    </>;
}
